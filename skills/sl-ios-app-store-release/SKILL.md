...
### 4.4.1 CLI 签名恢复：No Accounts / No iOS Distribution
...
```bash
asc publish appstore --app "$APP_ID" --ipa "./build/ipa/<scheme>.ipa" --wait --confirm
```
...
```bash
asc xcode validate --ipa "./build/ipa/<scheme>.ipa" \
  --api-key "$ASC_KEY_ID" --api-issuer "$ASC_ISSUER_ID"

asc builds upload --app "$APP_ID" --ipa "./build/ipa/<scheme>.ipa" \
...
2. **后续 release** (v1.1, v1.2...)：跳过阶段 1-2（已就绪），从阶段 3 重新生成 metadata，阶段 4-5 重新 archive + upload，阶段 6 通常只需要 update screenshots + 写 whatsNew（这次能写了）
3. **遇到错误**：先查 `references/common-pitfalls.md`；命令报错先看 `asc <command> --help`
4. **Submit 前**：必须人工确认。不要在 batch 脚本里自动 `asc review submit`。
